...
upload_before_timedelta = timedelta(hours=2)
...
def scan(camera_name, camera_dir, rclone_remote):
    upload_before_dt = datetime.now(tzinfo) - upload_before_timedelta
        ...
        if date_match:
            ...
            for hour_dir in hour_dirs:
                if HOUR_REGEX.match(hour_dir):
                    dir_dt = datetime.strptime(date_dir + ' ' + hour_dir, '%Y-%m-%d %H').replace(tzinfo=tzinfo)
                    mp4s_dir = hours_dir + '/' + hour_dir

                    if upload_before_dt > dir_dt:
